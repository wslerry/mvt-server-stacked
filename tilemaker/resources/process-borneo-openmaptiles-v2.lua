...
function relation_scan_function()
    ...
    if relType == "boundary" and boundary == "administrative" then
        Accept()
        return
    end

    -- Accept national parks
    if boundary == "national_park" then
        Accept()
        return
    end

    -- Accept protected areas
    if boundary == "protected_area" then
        Accept()
        return
    end

    -- Accept nature reserves
    if leisure == "nature_reserve" then
        Accept()
        return
    end

    -- Accept forest boundaries
    if relType == "boundary" and landuse == "forest" then
        Accept()
        return
    end
end
...
function way_function()
    ...
    -- Check relations for park/protected area designations
    ...
    local isForestBoundary = false

    while true do
        local rel = NextRelation()
        if not rel then break end

        local relType = FindInRelation("type")
        local relBoundary = FindInRelation("boundary")
        local relLanduse = FindInRelation("landuse")
        local relLeisure = FindInRelation("leisure")

        -- Check for administrative boundaries
        if relType == "boundary" and relBoundary == "administrative" then
            isBoundary = true
            admin_level = math.min(admin_level, tonumber(FindInRelation("admin_level")) or 11)
        end

        -- Check for national parks
        if relBoundary == "national_park" then
            isNationalPark = true
        end

        -- Check for protected areas
        if relBoundary == "protected_area" then
            isProtectedArea = true
        end

        -- Check for nature reserves
        if relLeisure == "nature_reserve" then
            isNatureReserve = true
        end

        -- Check for forest boundaries
        if relType == "boundary" and relLanduse == "forest" then
            isForestBoundary = true
        end
    end

    -- Check for direct tags on ways
    ...
    -- PRIORITY 1: Process Parks (National Parks, Protected Areas, Nature Reserves)
    -- These should go to the 'park' layer, not landcover
    if isNationalPark or isProtectedArea or isNatureReserve then
        if isClosed then
            Layer("park", true)

            -- Determine park class
            local parkClass = "protected_area"
            if isNationalPark then
                parkClass = "national_park"
            elseif isNatureReserve then
                parkClass = "nature_reserve"
            end

            Attribute("class", parkClass)
            SetMinZoomByArea()
            SetNameAttributes()

            -- Don't process this feature further for landcover
            -- But we might still want to show it as forest if it has that tag
            -- So we DON'T return here - we let it also be processed as forest below if applicable
        end
    end

    -- PRIORITY 2: Process Forest Boundaries
    -- These go to 'landcover' layer as wood
    if isForestBoundary or (landuse == "forest" and isClosed) then
    ...
    -- But skip if already processed as a park
    if not (isNationalPark or isProtectedArea or isNatureReserve) then
        local l = landuse
