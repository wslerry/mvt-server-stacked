...
-- The height of one floor, in meters (estimation of average building floor height)
...
function relation_scan_function()
    local boundary_type = Find("boundary")
    local landuse_type = Find("landuse")
    local leisure_type = Find("leisure")
    local natural_type = Find("natural")
    local protect_class = Find("protect_class")

    -- Accept boundary relations for national parks, protected areas, and administrative boundaries
    if Find("type") == "boundary" then
        if boundary_type == "administrative" or
            boundary_type == "national_park" or
            boundary_type == "protected_area" then
            Accept()
        end
    end

    -- Accept multipolygon relations for landuse=forest and protected areas
    if Find("type") == "multipolygon" then
        if landuse_type == "forest" or
            leisure_type == "nature_reserve" or
            boundary_type == "national_park" or
            boundary_type == "protected_area" or
            protect_class == "1" or protect_class == "2" or protect_class == "3" or protect_class == "4" or protect_class == "5" or protect_class == "6" then
            Accept()
        end
    end

    -- Also accept boundary relations with specific protection classes
    if protect_class and (protect_class == "1" or protect_class == "2" or protect_class == "3" or protect_class == "4" or protect_class == "5" or protect_class == "6") then
        Accept()
    end
end
...
function write_to_transportation_layer(minzoom, highway_class)
    ...
    local ramp = linkValues[Find("highway")]
    if ramp then AttributeNumeric("ramp", 1) end

    -- Service
    local service = Find("service")
    if Find("highway") == "service" and service ~= "" then Attribute("service", service) end
...
function way_function()
    ...
    local isProtectedArea = false
    local isForest = false
    local isNatureReserve = false
    ...
    while true do
        ...
        local rel_type = FindInRelation("type")
        local rel_boundary = FindInRelation("boundary")
        local rel_landuse = FindInRelation("landuse")
        local rel_leisure = FindInRelation("leisure")
        local rel_natural = FindInRelation("natural")
        local rel_protect_class = FindInRelation("protect_class")

        if rel_type == "boundary" then
            isBoundary = true
            if rel_boundary == "administrative" then
                admin_level = math.min(admin_level, tonumber(FindInRelation("admin_level")) or 11)
            elseif rel_boundary == "national_park" then
                isNationalPark = true
        ...
        if rel_type == "multipolygon" then
            ...
            elseif rel_boundary == "protected_area" then
                isProtectedArea = true
            elseif rel_landuse == "forest" then
                isForest = true
            elseif rel_leisure == "nature_reserve" then
                isNatureReserve = true
            end
        end

        -- Check for protection classes
        if rel_protect_class and (rel_protect_class == "1" or rel_protect_class == "2" or rel_protect_class == "3" or
                rel_protect_class == "4" or rel_protect_class == "5" or rel_protect_class == "6") then
            isProtectedArea = true
        end
    end
    ...
    -- Check protect_class tag
    local protect_class = Find("protect_class")
    if protect_class and (protect_class == "1" or protect_class == "2" or protect_class == "3" or
            protect_class == "4" or protect_class == "5" or protect_class == "6") then
        isProtectedArea = true
    end
    ...
    -- Process national parks and protected areas - WRITE TO PARK LAYER
    if isNationalPark or boundary == "national_park" then
        Layer("park", true)
        SetMinZoomByArea()
        Attribute("class", "national_park")
        SetNameAttributes()
        write_name = true
    end

    if isProtectedArea or boundary == "protected_area" or isNatureReserve or leisure == "nature_reserve" then
        Layer("park", true)
        SetMinZoomByArea()
        Attribute("class", "protected_area")
        SetNameAttributes()
        write_name = true
    end

    -- Process forest boundaries as wood landcover
    if isForest or landuse == "forest" then
    ...
    elseif waterway == "boatyard" then
        Layer("landuse", isClosed); Attribute("class", "industrial"); MinZoom(12)
    elseif waterway == "dam" then
        Layer("building", isClosed)
    elseif waterway == "fuel" then
        Layer("landuse", isClosed); Attribute("class", "industrial"); MinZoom(14)
    end
